              ...
              <div>
                ...
                >
                  {progress.status.charAt(0).toUpperCase() +
                    progress.status.slice(1).replace('-', ' ')}
                </span>
              </div>
                  ...
                  {progress.documents.map((doc) => (
                    ...
                    >
                      <div className="flex items-center">
                        <div
                          className={`w-3 h-3 mr-3 rounded-full ${getStatusColor(doc.status)}`}
                        />
                        <span>{doc.type}</span>
                        {doc.filename && (
          ...
          <div className="space-y-2">
            {documents.map((doc) => (
              <div key={doc.id} className="flex items-center justify-between p-3 border rounded">
                ...
                <span
                  className={`px-2 py-1 rounded text-sm ${getStatusColor(doc.status)} text-white`}
                >
                  {doc.status}
                </span>
              </div>
